import { Obj, PH } from "./utils/types.ts"
...
type Props = (<T>(p: (string | number)[], obj: Obj<T> | ArrayLike<T>) => (T | undefined)[])
  & ((p: (string | number)[], obj?: PH) => Props_2)
  & (<T>(p: PH, obj: Obj<T> | ArrayLike<T>) => Props_1<T>)
  & ((p?: PH, obj?: PH) => Props)

function _props<T>(p: (string | number)[], obj: Obj<T> | ArrayLike<T>) {
