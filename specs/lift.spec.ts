...
const add4 = curry(4, function (a: number, b: number, c: number, d: number) {
  return a + b + c + d
})
const add5 = curry(5, function (a: number, b: number, c: number, d: number, e: number) {
  return a + b + c + d + e
})
...
describe('lift', () => {
  ...
  it('should produces a cross-product of array values', function() {
    eq(addN3([1, 2, 3], [1, 2], [1, 2, 3]), [3, 4, 5, 4, 5, 6, 4, 5, 6, 5, 6, 7, 5, 6, 7, 6, 7, 8]);
    eq(addN3([1], [2], [3]), [6]);
