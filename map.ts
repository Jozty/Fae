...
type Map_1<F extends Obj<T> | Func | T[], T, R> = ((fn: FuncArr1<T, R>) => Func | R[] | Obj<R>)
  & ((fn?: PH) => Map_1<F, T, R>)

type Map = (<F extends Obj<T> | Func | T[], T, R>(fn: FuncArr1<T, R>, functor: F) => Func | R[] | Obj<R>)
  & (<F extends Obj<T> | Func | T[], T, R>(fn: FuncArr1<T, R>, functor?: PH) => Map_2<F, T, R>)
...
function _functionMap<T, R>(fn: FuncArr1<T, R>, functor: Func): Func {
  return curryN(getFunctionLength(functor), function (this: any, ...args: any[]) {
    return fn.call(this, functor.apply(this, args))
  })
}

function _objectMap<T, R>(func: FuncArr1<T, R>, functor: Obj<T>): Obj<R> {
...
function _map<F extends Obj<T> | Func | T[], T, R>(fn: FuncArr1<T, R>, functor: F) {
