...
const nonZeroExamples = examplesWithStatus.filter((e) => e.status.code !== 0);

if (nonZeroExamples.length) {
  const statusInfo = nonZeroExamples.map((e) => {
...
const outputs = await Promise.all(executedExamples.map(async (e) => {
...
if (nonEmptyOutputExamples.length) {
  const outputInfo = nonEmptyOutputExamples.map((e) => {
